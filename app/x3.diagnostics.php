...
if($posted){
	...
	# content dir exists
		...
		if(!file_exists('./content/folders.json')){
			if(Helpers::refresh_folders()){
				...
				$critical .= addItem("danger", "Failed to create folders.json", "Failed to create <code>content/folders.json</code>" . (!is_writeable('./content') ? ' because your <code>/content</code> directory is not writeable.' : '.'));
			}
	...
	/*if(!extension_loaded('mcrypt') && !X3Config::$config["back"]["panel"]["use_db"]){
	...
	if(X3Config::$config["settings"]["preload"] && !file_exists($file_path.'/content/site.json')){
